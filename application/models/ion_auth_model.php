...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		// sha1
		if ($this->store_salt) {
			$db_password = sha1($password . $hash_password_db->email);
		} else {
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_column == 'username' && $this->username_check($username)) {
			$this->errors['name'] = 'This name is already exist.';
			return FALSE;
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, is_admin')
		...
		if ($this->is_time_locked_out($identity)) {
			...
			$this->errors['time_out'] = "Login timeout.";
		...
		//Hash something anyway, just to take up time
		$this->hash_password($password);
		$this->errors['email'] = "Your credential is wrong.";
	...
	public function set_session($user)
	{
		...
		$this->session->set_userdata('curUser', $session_data);
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, username, email, is_admin')
